...
public class KmzCreator extends XmlCreator
{
   ...
   private Uri exportKml()
   {
      if (mFileName.endsWith(".kmz") || mFileName.endsWith(".zip"))
      {
       ...
       //  setExportDirectoryPath(Constants1.getSdCardDirectory(mContext) + "tracks/"+mFileName.substring(0, mFileName.length() - 4));//updated by ranvir
         
      }
      else
      {
         setExportDirectoryPath(Constants.getSdCardDirectory(mContext) + mFileName);
        // setExportDirectoryPath(Constants1.getSdCardDirectory(mContext) + "tracks/"+mFileName);//updated by ranvir
      }
